# 前端功能需求 v0.2（给 Gemini）

> 目标：Vue 3 前端对接 FastAPI 后端，完成 PDF 翻译任务创建、实时进度、结果预览。
> 约束：遵循 `docs/design/frontend-backend-contract-v0.2.md` 中“不可变约定”。
...
- 每条任务展示：文件夹名（PDF 文件名 stem，必要时带短后缀）、状态、创建时间、是否有 mono/dual
...
详见：`docs/design/frontend-backend-contract-v0.2.md` -> **0. 不可变约定**
